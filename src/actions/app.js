...
const loadPage = (page, query, bookId) => async (dispatch, getState) => {
    ...
    switch (page) {
        ...
        case "detail":
            ...
            break;
        case "news-detail":
            module = await import("../components/news-detail.js");
            // Fetch the book info for the given book id.
            await dispatch(module.getANews(bookId));
            // Wait for to check if the book id is valid.
            if (isFetchANewsFailed(getState().news)) {
                page = "404";
            }
            break;
        case "viewer":
        ...
        case "favorites":
            ...
            break;
        case "news":
            module = await import("../components/news-explore.js");
            if (query) {
                dispatch(module.searchNews(query));
            }
            break;
        case "about":
...
const isFetchANewsFailed = aNews => {
    if (!aNews) {
        updatePage("news");
    }
    return aNews && !aNews.isFetching && aNews.failure;
};
