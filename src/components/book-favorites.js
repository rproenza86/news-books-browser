    ...
    _render({ _items, _authInitialized, _user, _showOffline }) {
      ...
      <section hidden?="${_showOffline}">
        <div class="favorites-section" hidden?="${!_authInitialized || !_user}">
          <div class="favorites-empty" hidden?="${!_authInitialized ||
              !_items ||
              _items.length}">
          ...
          <ul class="books">
            ${_items &&
              ...
              <li>
                <book-item item="${item}">
                  <button class="fav-button" title="Remove favorite" on-click="${e =>
                      this._removeFavorite(e, item)}">${closeIcon}</button>
                </book-item>
              </li>
            `
                )}
        ...
        <div class="signin-section" hidden?="${!_authInitialized || _user}">
          ...
          <button class="book-button" on-click="${() =>
              store.dispatch(signIn())}">Sign in</button>
        </div>
      </section>
