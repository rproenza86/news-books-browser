...
class NewsBooksApp extends connect(store)(LitElement) {
    _render({
        appTitle,
        _page,
        ...
        const inputAtTop =
            ...
            !_wideLayout ||
            (_page === "explore" && _query) ||
            (_page === "news" && _query) ||
            (_page === "news-detail" && _query) ||
            _page === "detail" ||
            _page === "viewer";
        // back button href
        const backHref =
    ...
    <app-header condenses reveals effects="waterfall">
      <app-toolbar class="toolbar-top">
        ...
        <button class="signin-btn" aria-label="Sign In" visible?="${_authInitialized}"
            on-click="${() =>
                store.dispatch(
                    _user && _user.imageUrl ? signOut() : signIn()
                )}">
          ${
              _user && _user.imageUrl
                  ? html`<img src="${_user.imageUrl}">`
                  : accountIcon
          }
        </button>
      </app-toolbar>
          ...
          <input slot="input" id="input" aria-label="Search Books" autofocus type="search" value="${query}"
              on-change="${e => {
                  switch (_page) {
                      case "news":
                          store.dispatch(
                              updateLocationURL(`/news?q=${e.target.value}`)
                          );
                          break;
                      default:
                          store.dispatch(
    ...
    <app-drawer opened="${_drawerOpened}" hidden?="${!_lazyResourcesLoaded}"
      ...
      <nav class="drawer-list" on-click="${e =>
          store.dispatch(updateDrawerState(false))}">
        <a selected?="${_page === "news"}" href="/news${
            query ? "?q=" + query : ""
        }">News browser</a>
        <a selected?="${_page ===
            "explore"}" href="/explore?q=${query}">Books browser</a>
        <a selected?="${_page ===
            "favorites"}" href="/favorites">Favorites's Books</a>
    ...
    <main role="main" class="main-content">
      ...
      <book-explore class="_page" active?="${_page ===
      ...
      <book-favorites class="_page" active?="${_page ===
          "favorites"}"></book-favorites>
      <book-about class="_page" active?="${_page === "about"}"></book-about>
      <book-404 class="_page" active?="${_page === "404"}"></book-404>
      <news-explore class="_page" active?="${_page === "news"}"></news-explore>
      <news-detail class="_page" active?="${_page ===
          "news-detail"}"></news-detail>
    </main>

    <footer>
      <p>Copyrights © 2018 All Rights Reserved by Atomic Coders LLC.</p>
    </footer>
    ...
    _firstRendered() {
        ...
        installMediaQueryWatcher(
            `(min-width: 648px) and (min-height: 648px)`,
    ...
    _micResult(e) {
        ...
        if (d.isFinal) {
            if (this._page === "news") {
                store.dispatch(updateLocationURL(`/news?q=${value}`));
            } else {
...
window.customElements.define("news-books-app", NewsBooksApp);
